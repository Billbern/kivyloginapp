from kivy.lang import Builder
from db import MainDataB, Users
from kivy.uix.screenmanager import Screen


class SignupScreen(Screen):
    def __init__(self, name, **kwargs):
        super(SignupScreen, self).__init__(**kwargs)
        self.name = name

    def signup(self):
        self.ids.id_label.text = " "
        account = Users(user_id=None, email=self.ids.id_email.text, fname=self.ids.id_name.text,
                        password=self.ids.id_pass.text)
        if len(self.ids.id_name.text) < 1 or len(self.ids.id_email.text) < 1 or len(self.ids.id_pass.text) < 1 or\
                len(self.ids.id_conf.text) < 1:
            self.ids.id_label.text = "Please fill all Spaces"
        else:
            if self.ids.id_pass.text != self.ids.id_conf.text:
                self.ids.id_label.text = "Passwords don't match"
            else:
                if MainDataB.database.users.find_one({'email': self.ids.id_email.text}):
                    self.ids.id_label.text = "Email is already in use"
                else:
                    MainDataB.database.users.insert(account.get_as_json())
                    self.ids.id_label.text = "Account Created"
                    self.parent.current = "Signin"

    Builder.load_string("""
<SignupScreen>:
    FloatLayout:
        pos_hint: {"top": 1, 'left': 1}
        size_hint: None, None
        width: self.parent.width
        height: self.parent.height
        canvas:
            Color:
                rgb: utils.get_color_from_hex("#ffffff")
            Rectangle:
                size: self.size
                pos: self.pos
        
        ThemedImageButton:
            source: "./data/arrow.png"
            pos: "11dp", "605dp"
            on_release: root.parent.current= 'Signin'
            
        Label:
            text: '[color=34ace0][b]Signup[/b][/color]'
            markup: True
            pos: "142dp", "524dp"
            font_size: '24sp'
            size_hint: None, None
            width: "76dp"
            height: "32dp"
        
        Label:
            id: id_label
            color: utils.get_color_from_hex("#b33939")
            pos: "53dp", "443dp"
            font_size: '12sp'
            size_hint: None, None
            width: '259dp'
            height: '48dp'
            
        ThemedInput:
            id: id_name
            hint_text: "Full Name"
            pos: "50.5dp", "396dp"
            
        ThemedInput:
            id: id_email
            hint_text: "Email"
            pos: "50.5dp", "320dp"

        ThemedInput:
            id: id_pass
            hint_text: "Password"
            password: True
            pos: "50.5dp", "244dp"

        ThemedInput:
            id: id_conf
            hint_text: "Confirm Password"
            password: True
            pos: "50.5dp", "168dp"
            
        ThemedButton:
            text: '[b]Signup[/b]'
            pos: "50.5dp", "94dp"
            on_release: root.signup()

    """)
